...
interface TwitchService {
    ...
    @Headers("Client-ID: $CLIENT_ID", "Accept: application/vnd.twitchtv.v5+json")
    suspend fun getChannelSearches(@Query("query") query: String, @Query("limit") limit: Int, @Query("offset") offset: Int,  @Query("api_version") version: Int = 5) : Response<List<SearchViewLayout.SearchData>?>

    @GET("kraken/search/games")
    @Headers("Client-ID: $CLIENT_ID", "Accept: application/vnd.twitchtv.v5+json")
    suspend fun getGamesSearches(@Query("query") query: String, @Query("limit") limit: Int = 10,  @Query("api_version") version: Int = 5) : Response<List<GameSearchesAdapter.GamesSearchData>?>
